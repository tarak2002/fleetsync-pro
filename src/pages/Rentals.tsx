    ...
    const handleVehicleChange = (vehicleId: string) => {
        ...
        if (vehicle) {
            setFormData({
                ...
                bondAmount: vehicle.bond_amount,
            });
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await rentalsApi.create({
                ...
                bond_amount: formData.bondAmount,
                                ...
                                >
                                    ...
                                    {availableVehicles.map((v) => (
                                        <option key={v.id} value={v.id}>
                                            {v.plate} - {v.make} {v.model} ({formatCurrency(v.weekly_rate)}/wk)
                                        </option>
                                    ))}
                                </Select>
                                ...
                                {loading ? (
                                    ...
                                    rentals.map((rental) => (
                                        <tr key={rental.id} className="border-b border-slate-100 hover:bg-slate-50">
                                            ...
                                            <td className="p-4">{formatDate(rental.start_date)}</td>
                                            <td className="p-4">{formatDate(rental.next_payment_date)}</td>
                                            <td className="p-4 text-right font-medium">{formatCurrency(rental.weekly_rate)}</td>
